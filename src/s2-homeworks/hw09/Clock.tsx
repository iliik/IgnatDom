...
function Clock() {
    ...
    const start = () => {
        ...
        let time = window.setInterval(() => setDate(new Date()), 1000)
        setTimerId(time)
    }
    ...
    const stringTime = `${hours < 10 ? "0" + hours : hours}:${min < 10 ? "0" + min : min}:${secs < 10 ? "0" + secs : secs}` ||
    ...
    let formatter2 = new Intl.DateTimeFormat('en', {weekday: "long"})
    const stringDay = formatter2.format(date) || <br/> // пишут студенты

    let formatter3 = new Intl.DateTimeFormat("en", {month: "long"})
    const stringMonth = formatter3.format(date) || <br/> // пишут студентыы
